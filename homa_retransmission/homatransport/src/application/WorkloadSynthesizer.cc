...
int testFlowNum = 10; // run very slowly
int testFinish = false; // do not change

bool enableWriteFile = false; // output fct size
bool outFctFileOpen = false; // do not change

bool enableWriteRatio = false;
bool outRatioFileOpen = false; // do not change
...
WorkloadSynthesizer::getRandomIncastDest (int* destHostId, int localHostId) {
    // std::cout << "periodFinished? " << periodFinished << endl;
    if (periodFinished) { // generate new one
        ...
        // std::cout << "choose receiver, hostid=" << incastReceiver << endl;
        // choose senders
...
//simsignal_t WorkloadSynthesizer::KBMsgSendSignal = registerSignal("KBMsgSend");
simsignal_t WorkloadSynthesizer::KBMsgReceivedSignal = registerSignal("KBMsgReceived");

//simsignal_t WorkloadSynthesizer::KBMsgSendBytesSignal = registerSignal("KBMsgSendBytes");
// simsignal_t WorkloadSynthesizer::KBMsgReceivedBytesSignal = registerSignal("KBMsgReceivedBytes");
...
WorkloadSynthesizer::WorkloadSynthesizer()
{
    ...
    if (enableWriteRatio && !outRatioFileOpen) {
        outRatioFile.open (ratioFileName, std::ios::out|std::ios::trunc);
        outRatioFileOpen = true;
    }
}
...
WorkloadSynthesizer::processIncast() 
{
   ...
   //  destAddrs
    ...
    if (!getDestFlag) {
        // std::cout << "This host isn't the incast sender, ignored" << endl;
...
WorkloadSynthesizer::processRcvdMsg(cPacket* msg)
{
    ...
    if(msgByteLen <= 100000){
        ...
        emit(KBMsgReceivedSignal, 1);
        // emit(KBMsgReceivedBytesSignal, msgByteLen);
    }
    ...
    // emit(msgCompleteSizeSignal, msgByteLen);
