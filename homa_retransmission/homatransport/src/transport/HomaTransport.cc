...
std::string loadFileName = "susload/tmp.5_homa.txt" ;
std::ofstream outLoadFile;

std::string ratioFileName = "fct_result/tmp.txt" ;
std::ofstream outRatioFile; // recv unrepeated pkt / send pkt
uint64_t byteSend = 0;
uint64_t byteRecv = 0;

std::ofstream outFctFile;
std::string fctFileName = "fct_result/tmp.txt" ; 

bool enableRESEND = true; // false, not set timer
bool enableCutOff = true; 
int maxRxTimerCount = 50;
...
simsignal_t HomaTransport::throughPutSignal =
        registerSignal("throughPut");  
simsignal_t HomaTransport::goodputSizeSignal =
        registerSignal("goodputSize");     

simsignal_t HomaTransport::KBMsgSendSignal =
        registerSignal("KBMsgSend");    
...
HomaTransport::handleRecvdPkt(cPacket* pkt)
{
    ...
    switch (rxPkt->getPktType()) {
        ...
        case PktType::GRANT:
            ...
            break;
        case PktType::RESEND:
            sxController.processReceivedResend(rxPkt);
            break;
    ...
    PriorityResolver* prioResolver)
{
    ...
    msgTimeoutValSender = SimTime(0.1); // unit: s
...
HomaTransport::SendController::processSendMsgFromApp(AppMessage* sendMsg)
{
    ...
    if (msgSize <= 100000){
        transport->emit(KBMsgSendSignal, 1);
    }
...
HomaTransport::SendController::processReceivedGrant(HomaPkt* rxPkt)
{
    ...
    //When received a GRANT, corresponding Msg's Timer should be canceled.
    ...
    int bytesToSchedNew = outboundMsg->prepareSchedPkt(
        ...
        rxPkt->getGrantFields().schedPrio, 0);
...
//process RESEND pkt received
void
HomaTransport::SendController::processReceivedResend(HomaPkt* rxPkt)
{
    //When received a RESEND, corresponding Msg's Timer should be canceled.
    auto iter = msgTimeoutSenderTimerMap.find(rxPkt->getMsgId());
    if (iter != msgTimeoutSenderTimerMap.end()) {
        cMessage* msgTimeoutTimer = msgTimeoutSenderTimerMap[rxPkt->getMsgId()];
        msgTimeoutSenderTimerToMsgIdMap.erase(msgTimeoutTimer);
        transport->cancelAndDelete(msgTimeoutTimer);
        msgTimeoutSenderTimerMap.erase(rxPkt->getMsgId());
    }    
    ...
    size_t numRemoved = outbndMsgSet.erase(outboundMsg);
    // ASSERT(numRemoved <= 1); // At most one item removed
    this->bytesLeftToSend += rxPkt->getResendFields().resendBytes;
    int bytesToSchedNew = outboundMsg->prepareSchedPkt(
        rxPkt->getResendFields().offset, rxPkt->getResendFields().resendBytes,
        0, 1);//rxPkt->getResendFields().schedPrio

    auto insResult = outbndMsgSet.insert(outboundMsg);
    // ASSERT(insResult.second); // check insertion took place
    outboundMsg->bytesLeft += rxPkt->getResendFields().resendBytes;
    delete rxPkt;
    sendOrQueue();
}
...
HomaTransport::SendController::processReceivedMsgAck(HomaPkt* rxPkt) {
    ...
    // erase SxTimer
    auto iter = msgTimeoutSenderTimerMap.find(msgId);
    if (iter != msgTimeoutSenderTimerMap.end()) {
        cMessage* msgTimeoutTimer = msgTimeoutSenderTimerMap[msgId];
        msgTimeoutSenderTimerToMsgIdMap.erase(msgTimeoutTimer);
        transport->cancelAndDelete(msgTimeoutTimer);
        msgTimeoutSenderTimerMap.erase(msgId);
    }
    return;
}

//Called by HomaTransport::handleMessage
//Used to handle SelfMsgKind::MSGTIMEOUTSENDER
//Param: Timer
void
HomaTransport::SendController::processMsgTimeoutSenderTimers(cMessage* msgTimeoutSenderTimer)
{
    ...
    if (outboundMsgMap.find(msgId) == outboundMsgMap.end()){
        // std::cout<<"[debug SXTIMER] timer's msg has removed from outboundMsgMap"<<endl;
        return;
    }
    OutboundMessage* outboundMsg = &(outboundMsgMap.at(msgId));
    outboundMsg->sxTimerCounter += 1;
    ...
    auto insResult = outbndMsgSet.insert(outboundMsg);
    // set sxtimer again
    this->transport->scheduleAt(simTime() + msgTimeoutValSender, msgTimeoutSenderTimer);  

    // ASSERT(insResult.second); // check insertion took place
    //add timeout for msg retransmission at sender
    transport->scheduleAt(simTime() + msgTimeoutValSender, msgTimeoutSenderTimer);
    sendOrQueue();
    // std::cout<<"[debug SXTIMER] Sxtimer process sucess"<<endl;
}
...
HomaTransport::SendController::sendOrQueue(cMessage* msg)
{
    ...
    if (sxPkt) {
        ...
        else if( sxPkt->getPktType() == PktType::RESEND || sxPkt->getPktType() == PktType::MSG_ACK ){
            if(transport->sendTimer->isScheduled()){
                outGrantQueue.push(sxPkt);
                return;
            }else{
...
HomaTransport::SendController::sendPktAndScheduleNext(HomaPkt* sxPkt)
{
    ...
    switch((PktType)sxPkt->getPktType())
    {
        case PktType::REQUEST:
        case PktType::UNSCHED_DATA:
        case PktType::SCHED_DATA:
		// std::cout << "pkt type: " << pktType << std::endl;
		// std::cout << bytesLeftToSend << " " << numDataBytes << std::endl;
        // if(bytesLeftToSend < numDataBytes) {
        //     std::cout << "src " << sxPkt->getSrcAddr() <<std::endl;
        // }
        // if(sxPkt->getSrcAddr() == inet::L3Address("10.0.3.9")) {
        //     std::cout << bytesLeftToSend << std::endl;
        //     std::cout << "numDataBytes " << numDataBytes << std::endl;
        //     std::cout << "getFirstByte " << sxPkt->getFirstByte() << std::endl;
        //     std::cout << "pkt type " << pktType << std::endl;
        // }
        // if(resendPktSet.find(sxPkt) != resendPktSet.end()) {
        //     std::cout << "resend pkt " << std::endl;
        //     std::cout << "src" << sxPkt->getSrcAddr() <<std::endl;
        // }
	    ASSERT(bytesLeftToSend >= numDataBytes);
            if (enableWriteRatio) {
                byteSend += numDataBytes; 
            }
            if(resendPktSet.find(sxPkt) == resendPktSet.end()){// not resend pkt
                bytesLeftToSend -= numDataBytes;                
            } else {
                resendPktSet.erase(sxPkt);
                std::cout << "resend pkt " << std::endl;
                std::cout << "src" << sxPkt->getSrcAddr() <<std::endl;
            }
    ...
    transport->emit(throughPutSignal, bytesSentOnWire);
...
HomaTransport::SendController::msgTransmitComplete(OutboundMessage* msg)
{
    ...
    if (rxAddrMsgMap.at(destIp).empty()) {
        ...
        // influence the statistic in handlePktTransmitEnd()
    }
    transport->testAndEmitStabilitySignal();
    return;
}
...
HomaTransport::OutboundMessage::prepareRequestAndUnsched()
{
    ...
    // if(this->srcAddr == inet::L3Address("10.0.3.9")) {
    //         std::cout << "totalUnschedBytes: " << totalUnschedBytes << std::endl;
    //         // std::cout << "numDataBytes " << numDataBytes << std::endl;
    //         // std::cout << "getFirstByte " << sxPkt->getFirstByte() << std::endl;
    //         // std::cout << "pkt type " << pktType << std::endl;
    //     }
 ...
 * \param type
 *         0 for grant and 1 for resend
 * \return
 *      remaining scheduled bytes to be sent for this message.
 */
uint32_t
HomaTransport::OutboundMessage::prepareSchedPkt(uint32_t offset,
    uint16_t numBytes, uint16_t schedPrio, int type)
{

    if (offset == 0 && type == 1) {
        uint16_t leftResendByte = numBytes;
        while (leftResendByte) {
            HomaPkt* unschedPkt = new HomaPkt(sxController->transport);
            ...
            unschedPkt->setSrcAddr(this->srcAddr);
            unschedPkt->setMsgId(this->msgId);
            unschedPkt->setPriority(0);
            
            UnschedFields unschedFields;
            uint16_t thisPacketSize = std::min (leftResendByte, (uint16_t)1456);
            leftResendByte = leftResendByte - thisPacketSize;
            unschedFields.msgByteLen = thisPacketSize;
            unschedFields.msgCreationTime = msgCreationTime;
            unschedFields.firstByte = 0;
            unschedFields.lastByte = thisPacketSize - 1;
    ...
    uint32_t bytesToSend = (uint32_t)numBytes;
    ...
    if(type == 1) { // retransmission
        sxController->resendPktSet.insert(dataPkt);   
    }
    // update outbound messgae
    if(type == 0) // original type
...
HomaTransport::ReceiveScheduler::ReceiveScheduler(HomaTransport* transport)
    ...
    , msgTimeoutValReceiver(SimTime(0.01)) // 41us 0.000041 // 10ms 0.01
    , msgReceived(0)
    , msgFinished(0)
...
{
    // std::cout<<"[debug] enter processReceivedPkt(), msgID: "<<rxPkt->getMsgId()<<", type: "<<rxPkt->getPktType()<<endl;
    uint32_t pktLenOnWire = HomaPkt::getBytesOnWire(rxPkt->getDataBytes(),
        (PktType)rxPkt->getPktType());
    simtime_t pktDuration =
        SimTime(1e-9 * (pktLenOnWire * 8.0 / homaConfig->nicLinkSpeed));
    ...
    *sumGrantsInGap = 0;
    ...
    if (iter == ipSendersMap.end()) {
        ...
        if(rxPkt->getPktType() == PktType::SCHED_DATA){
            // std::cout<<"!! non-request pkt want to build new sendstate, type: "<<rxPkt->getPktType()<<", msgId: "<<rxPkt->getMsgId()<<endl;
            return;
        }
        char timerName[50];
    ...
    int sIndOld;
    try{
        sIndOld = schedSenders->remove(s);
    }catch(...){  
        return;
    }
    if(sIndOld == -10)  return; // -10 magic number, "Trying to remove an unlisted sched sender"
    ...
    if( msgCompHandle.first == false && msgCompHandle.second == 10086){ // 10086 magic number
        outofOrder = true;
    }

    if (enableWriteRatio && !outofOrder) {
        byteRecv += rxPkt->getDataBytes();
        outputCount += 1;
        outputCount %= outputInterval;  
        if (outputCount == 0) {
            outRatioFile << (double)byteRecv/(double)byteSend
                <<"\t" << byteRecv
                <<"\t" << byteSend
                << endl;
        } 
    ...
    // not repeated pkt, count goodput
    if (enableSustainable && !outofOrder) {
    ...
    // this->transport->emit(goodputSizeSignal, pktLenOnWire);
    this->transport->emit(goodputSizeSignal, rxPkt->getDataBytes());
...
//Called by HomaTransport::handleMessage()
//Used to make and send RESEND pkt
//Param: Timer
void
HomaTransport::ReceiveScheduler::processRxTimers(cMessage* msgTimeoutReceiverTimer)
{
    if(msgTimeoutReceiverTimerMap.find(msgTimeoutReceiverTimer) == msgTimeoutReceiverTimerMap.end()){
        this->transport->cancelAndDelete(msgTimeoutReceiverTimer);
        return;
    }
    
    SenderState* s = msgTimeoutReceiverTimerMap[msgTimeoutReceiverTimer];
    s->handleMsgTimeoutReceiverTimerEvent(msgTimeoutReceiverTimer);
}


void
HomaTransport::ReceiveScheduler::SenderState::sendMsgAck(InboundMessage* inboundMessage) {
    HomaPkt* msgAckPkt = inboundMessage->prepareMsgAck();
    this->rxScheduler->transport->sxController.sendOrQueue(msgAckPkt);
}

void
HomaTransport::ReceiveScheduler::SenderState::handleMsgTimeoutReceiverTimerEvent(
    cMessage* msgTimeoutReceiverTimer)
{
    if (!enableRESEND) { // ignore this msg
        return;
    }
    uint64_t msgId = RxTimerMsgIDMap[msgTimeoutReceiverTimer];

    auto rxPktMsgID = msgId;
    auto timeriter = MsgIDRxTimerMap.find(rxPktMsgID);
    //if exist, find and delete, then add a new timer

    if(timeriter != MsgIDRxTimerMap.end()){
        if (msgTimeoutReceiverTimer->isScheduled()) {
            this->rxScheduler->transport->cancelEvent(msgTimeoutReceiverTimer);
        }
    }
    this->rxScheduler->transport->scheduleAt(simTime() + this->rxScheduler->msgTimeoutValReceiver, msgTimeoutReceiverTimer);

    if(enableRESEND){
        // std::cout<<"[debug RESEND] enableRESEND, ready to send RESEND "<<endl;
        InboundMessage* inboundMesg = NULL;
        auto iter_msg = incompleteMesgs.find(msgId);
        if( iter_msg == incompleteMesgs.end() ){
            // std::cout<<"[debug RESEND] timeout msg has already completed"<<endl;
            return;
        }
        inboundMesg = iter_msg->second;
        inboundMesg->rxTimerCounter += 1;

        if (enableCutOff) {
            if (inboundMesg->rxTimerCounter == maxRxTimerCount) {
                // treat this msg as finished
                InboundMessage* timeoutMsg = inboundMesg;
                inboundMesg->rxTimerCounter += 1;
            ...
            if (inboundMesg->rxTimerCounter >= maxRxTimerCount+3) {
                if (msgTimeoutReceiverTimer->isScheduled()) {
                    this->rxScheduler->transport->cancelEvent(msgTimeoutReceiverTimer);
                }
                return;
        ...
        int resendType = 1; // 0 for first grant, 1 for first range, 2 for all range
        uint32_t offSetByte;
        uint16_t sizeGrant;
        simtime_t grantSentTime;

        uint32_t nextOffSetByte;
        uint16_t nextSizeGrant;

        // HomaPkt* resendPkt;
        /** "if(offSetByte == 0)" part, implement in "resendType=0"
         */
        if(resendType == 0){
            std::tie(offSetByte, sizeGrant, grantSentTime) = inboundMesg->inflightGrants.front();
            if(offSetByte == 0){    //if grant respond to unscheduled, offset = 0, ignored;
                offSetByte = 0;
            }
            HomaPkt* resendPkt = inboundMesg->MakeResend(offSetByte, sizeGrant, grantSentTime);
            this->rxScheduler->transport->sxController.sendOrQueue(resendPkt);
        }
        else if(resendType == 1){
            std::list<std::tuple<uint32_t, uint16_t, simtime_t>>::iterator iter = inboundMesg->inflightGrants.begin();
            while(iter!=inboundMesg->inflightGrants.end()){
                std::tie(offSetByte, sizeGrant, grantSentTime) = *iter;
                HomaPkt* resendPkt = inboundMesg->MakeResend(offSetByte, sizeGrant, grantSentTime);
                this->rxScheduler->transport->sxController.sendOrQueue(resendPkt);

                iter++;
                std::tie(nextOffSetByte, nextSizeGrant, grantSentTime) = *iter;
                if(nextOffSetByte != offSetByte + sizeGrant){
                    break;
                }
            }            
        }
            ...
            std::cout<<"[debug] resend Type error"<<endl;
        }
        // std::cout<<"[debug RESEND] process Rxtimer success"<<endl;
    }
}

//Called by HomaTransport::ReceiveScheduler::processRxTimers()
//Used to make and send RESEND pkt
//Param: Timer
HomaPkt*
HomaTransport::InboundMessage::MakeResend( uint32_t offSetByte, uint16_t sizeGrant, simtime_t grantSentTime){
    HomaPkt* resendPkt = new HomaPkt(rxScheduler->transport);
    resendPkt->setPktType(PktType::RESEND);
    ResendFields resendFields;
    resendFields.offset = offSetByte;
    resendFields.resendBytes = sizeGrant;
    resendPkt->setResendFields(resendFields);
    resendPkt->setDestAddr(this->srcAddr);
    resendPkt->setSrcAddr(this->destAddr);
    resendPkt->setMsgId(this->msgIdAtSender);
    resendPkt->setByteLength(resendPkt->headerSize());
    resendPkt->setPriority(0);
    return resendPkt;
}
...
HomaTransport::InboundMessage::prepareMsgAck () {
    ...
    ackPkt->setPktType(PktType::MSG_ACK);
...
{
    bool outofOrder = false; // true if rxPkt is repeated pkt and ignore it.
    ...
    if (mesgIt == incompleteMesgs.end()) {
        // ASSERT(rxPkt->getPktType() == PktType::REQUEST || rxPkt->getPktType() == PktType::UNSCHED_DATA);
        if(rxPkt->getPktType() != PktType::REQUEST && rxPkt->getPktType() != PktType::UNSCHED_DATA){
            //repeated pkt
            ret.first = false;
            ret.second = 10086; // masgic number here
            return ret;
        }
        if (this->msgReceivedSenderSet.count(rxPkt->getMsgId())) {
            ret.first = false;
            ret.second = 10086;
            return ret;
        }
        ...
        this->rxScheduler->msgReceived += 1;
        this->rxScheduler->msgReceivedSet.insert (rxPkt->getMsgId());
        this->msgReceivedSenderSet.insert (rxPkt->getMsgId());
    ...
    //need to check whether pkt is repeated
    ...
    if (pktType == PktType::REQUEST || pktType == PktType::UNSCHED_DATA) {
        whetherRepeat = inboundMesg->fillinRxBytes(rxPkt->getUnschedFields().firstByte, rxPkt->getUnschedFields().lastByte, pktType);
    } else if (pktType == PktType::SCHED_DATA) {
        whetherRepeat = inboundMesg->fillinRxBytes(rxPkt->getSchedDataFields().firstByte, rxPkt->getSchedDataFields().lastByte, pktType);
    } else {
        throw cRuntimeError("PktType %d is not recongnized",
            rxPkt->getPktType());
    }
    if(whetherRepeat == true){
        outofOrder = true;
    }
    //not repeated pkt can arrive here and need to bind or update timer.
    //build new rx timer and bind to the new msg
    auto rxPktMsgID = rxPkt->getMsgId();
    if (enableRESEND && !outofOrder) {
        auto timeriter = this->MsgIDRxTimerMap.find(rxPktMsgID);
        //if exist, find and delete, then add a new timer
        cMessage* msgTimeoutTimer = NULL;
        int timerExist = 0;
        if(timeriter != this->MsgIDRxTimerMap.end()){
            msgTimeoutTimer = this->MsgIDRxTimerMap[rxPktMsgID];
            timerExist = 1;
        }
        if (!timerExist) { // create a new one
            msgTimeoutTimer = new cMessage("msgTimeoutReceiverTimer");
            msgTimeoutTimer->setKind(SelfMsgKind::MSGTIMEOUTRECEIVER);
            this->MsgIDRxTimerMap[rxPktMsgID] = msgTimeoutTimer;
            this->RxTimerMsgIDMap[msgTimeoutTimer] = rxPktMsgID;
            this->rxScheduler->msgTimeoutReceiverTimerMap[msgTimeoutTimer] = this;
            // std::cout << "create rx timer for msgid=" << rxPktMsgID << endl;
        }

        if (msgTimeoutTimer->isScheduled()) {
            this->rxScheduler->transport->cancelEvent(msgTimeoutTimer);
        }
        // the only place to set the rxtimer
        if (enableCutOff) {
            if (inboundMesg->rxTimerCounter <= maxRxTimerCount) {
                this->rxScheduler->transport->scheduleAt(simTime() + this->rxScheduler->msgTimeoutValReceiver, msgTimeoutTimer);
            }
        } else {
            this->rxScheduler->transport->scheduleAt(simTime() + this->rxScheduler->msgTimeoutValReceiver, msgTimeoutTimer);
        }
    ...
    if (inboundMesg->bytesToReceive == 0) {
        ...
        cMessage * oldMsgTimeoutReceiverTimer = this->MsgIDRxTimerMap[inboundMesg->msgIdAtSender];
        this->MsgIDRxTimerMap.erase(inboundMesg->msgIdAtSender);
        this->RxTimerMsgIDMap.erase(oldMsgTimeoutReceiverTimer);
        this->rxScheduler->msgTimeoutReceiverTimerMap.erase(oldMsgTimeoutReceiverTimer);
        this->rxScheduler->transport->cancelAndDelete(oldMsgTimeoutReceiverTimer);

        AppMessage* rxMsg = inboundMesg->prepareRxMsgForApp();

        this->rxScheduler->msgFinished += 1;
        
        if (this->rxScheduler->transport->enableLogRxTimer) {
            for (auto iter : this->rxScheduler->ipSendersMap) {
                SenderState* ip = std::get<1>(iter);
                if (ip->incompleteMesgs.size()) {
                    InboundMessage* unfinishedMessage = incompleteMesgs.begin()->second;
                    uint32_t offSetByte=-1; uint16_t sizeGrant=-1; simtime_t grantSentTime;
                    if (unfinishedMessage->inflightGrants.size()) {
                        std::tie(offSetByte, sizeGrant, grantSentTime) = unfinishedMessage->inflightGrants.front();
                        
                    }
                    float ratio = 1-(float)unfinishedMessage->bytesToReceive/unfinishedMessage->msgSize;
                    if (checkRatio && ratio > 0.9) {
                    std::cout<<"handleInboundPkt(): msgId="<<rxPktMsgID<<", left bytes="<<unfinishedMessage->bytesToReceive
                        <<"/"<<unfinishedMessage->msgSize
                        <<"(ratio="<<1-(float)unfinishedMessage->bytesToReceive/unfinishedMessage->msgSize<<")"
                        <<",totalUnschedBytes="<<unfinishedMessage->totalUnschedBytes
                        <<",inflightGrants.size="<<unfinishedMessage->inflightGrants.size()
        ...
        int left = 0;
        for (auto iter:this->rxScheduler->ipSendersMap) {
            SenderState* ss = std::get<1>(iter);
            if (ss->incompleteMesgs.size()) {
                left += 1;
            }
        }

        // notification to sender
        sendMsgAck(inboundMesg);
        ...
        uint32_t grantPrio)
{
    ...
    // cancel checking mind BDP. due to pkt loss, inflightbytes are not correct, which will reduce the throughput.
...
{
    // std::cout<<"[debug remove()] enter remove()"<<endl;
    if (numSenders == 0) {
        senders.clear();
        headIdx = schedPrios;
        for (size_t i = 0; i < schedPrios; i++) {
 ...
 * \return
 *      true for repeated pkt
        false for not repeated pkt
 */
bool
        ...
        uint32_t byteEnd, PktType pktType)
{
    ...
    if (pktType == PktType::UNSCHED_DATA || pktType == PktType::REQUEST) {
        ...
        if(inflightGrants.size() == 0){
            return true;
        }
        // ASSERT(std::get<0>(*grantZero) == 0);
        if(std::get<0>(*grantZero) != 0){   //repeated
            // std::cout<<"received unscheduled pkt but first grant.offset != 0, ignored"<<endl;
            return true;
        }
        // ASSERT(std::get<1>(*grantZero) >= bytesReceived);
        if(std::get<1>(*grantZero) < bytesReceived)
            bytesReceived = std::get<1>(*grantZero);
        std::get<1>(*grantZero) -= bytesReceived;
        if (std::get<1>(*grantZero) == 0) {
            // std::cout << "grant list erase uncheduled part, finished, left grant num="<<inflightGrants.size()<<endl;
    ...
    if (pktType == PktType::SCHED_DATA) {
        ...
        if(grant == inflightGrants.end()){   //repeated pkt, whose grant has already moved
            // std::cout<<"repeated scheduled pkt, whose grant has already moved"<<endl;
            return true; 
        }
        if(std::get<1>(*grant) != bytesReceived){
            // std::cout<<"!! error: grant size in list != bytesReceived"<<endl;
            return true;
        }
    ...
    // std::cout<<"[debug FILLIN] end fillinRxBytes()"<<endl;
    return false;
}
...
{
    // std::cout<<"[debug APP] finished msg, msgid: "<<this->msgIdAtSender<<", size: "<<this->msgSize<<endl;
