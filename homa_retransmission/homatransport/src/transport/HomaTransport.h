...
extern std::ofstream outRatioFile; // recv unrepeated pkt / send pkt
extern std::string ratioFileName;
extern bool enableWriteRatio;
extern bool outRatioFileOpen;
...
class HomaTransport : public cSimpleModule
{
  PUBLIC:
    ...
    class SendController
    {
      PUBLIC:
        ...
        void processReceivedResend (HomaPkt* rxPkt);
        ...
        // unordered_set<uint32_t> sxMsgTimeoutSet;
      ...
      PUBLIC:
        ...
        //set to track all resend pkt
        std::unordered_set<HomaPkt*> resendPktSet;
    ...
    class InboundMessage
    {
      ...
      PROTECTED:
        ...
        bool fillinRxBytes(uint32_t byteStart, uint32_t byteEnd,
            PktType pktType);
        uint32_t schedBytesInFlight();
        uint32_t unschedBytesInFlight();
        HomaPkt* prepareGrant(uint16_t grantSize, uint16_t schedPrio);

        HomaPkt* MakeResend( uint32_t offSetByte, uint16_t sizeGrant, simtime_t grantSentTime);
    ...
    class ReceiveScheduler
    {
      PUBLIC:
        ...
        void processRxTimers(cMessage* msgTimeoutReceiverTimer);
        uint16_t msgReceived;
        uint16_t msgFinished;
        std::unordered_set<uint64_t> msgReceivedSet;
        ...
        class SenderState { 
          ...
          PUBLIC:
            ...
            //map: MsgID -> Rx Timer
            std::unordered_map<uint64_t, cMessage*> MsgIDRxTimerMap;
            //map: Rx Timer -> MsgID
            std::unordered_map<cMessage*, uint64_t> RxTimerMsgIDMap;
            std::unordered_set<uint64_t> msgReceivedSenderSet;

            void handleMsgTimeoutReceiverTimerEvent(cMessage* msgTimeoutReceiverTimer); 
            void sendMsgAck(InboundMessage* inboundMessage);
            ...
            class CompSched {
              PUBLIC:
                ...
                bool operator()(const SenderState* lhs, const SenderState* rhs)
                {
                    ...
                    // std::cout<<"~"<<endl;
                    // std::cout<<"size1,2: "<<lhs->mesgsToGrant.size()<<" "<<rhs->mesgsToGrant.size()<<endl;
                    i = cbg(*lhs->mesgsToGrant.begin(),
                        *rhs->mesgsToGrant.begin());
                    // std::cout<<i<<endl;
                    return i;
                }
      ...
      PROTECTED:
        ...
        //timeout value for Pkt at receiver
  ...
  PUBLIC:
    ...
    enum SelfMsgKind
    {
        ...
        MSGTIMEOUTSENDER = 7, // Sent to Sender when it lost all pkts of a msg and retransmission it then.
        MSGTIMEOUTRECEIVER = 8 // Sent to Receiver when there aren't new Pkt of the Msg
  ...
  PUBLIC:
    // set to track all resend pkt
    // static std::unordered_set<HomaPkt*> resendPktSet; 

    // simtime_t resendInterval;
    ...
    static simsignal_t KBMsgSendSignal;

    static simsignal_t goodputSizeSignal;
