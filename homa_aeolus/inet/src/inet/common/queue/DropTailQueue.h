...
class INET_API DropTailQueue : public PassiveQueueBase
{
  protected:
    ...
    // variable to enable Aeolus
    bool enableAeolus;

    //selective dropping threshold
    int selectiveDropThres;

    //maximum queue length recorded
    int max_queue_length_recorded;

    //summed queue length recorded
    long long queue_length_sum;

    //counter to trigger print
    int queue_length_print_counter;
