...
class HomaConfigDepot {
  ...
  PUBLIC:
    ...
    uint64_t addr = (uint64_t)&sxScheme;

  PUBLIC:
    /**
     * getter method for SenderScheme enum-ed param.
     */
    const SenderScheme getSenderScheme() {
        // // std::cout << "sxScheme=" <<sxScheme<< endl;
        // if (addr != (uint64_t)&sxScheme) {
        //   addr = (uint64_t)&sxScheme;
        //   std::cout << "addr = " << addr << endl;
        // }
        // sxScheme = SenderScheme::SRBF;
        return sxScheme;
        // return SenderScheme::SRBF;
      }
