 ...
 */
extern bool enableSustainable;
extern int outputInterval;
extern int outputCount;
extern uint64_t totalByteRecv;
extern double recStart;
extern double recEnd;
extern double recLastFlow;
extern double totalGoodput;
extern double maxGoodput;


extern std::ofstream outLoadFile;
extern std::string loadFileName;
extern bool enableWriteLoad;
extern bool outLoadFileOpen;

extern std::ofstream outFctFile;
extern std::string fctFileName;
...
class HomaTransport : public cSimpleModule
{
  PUBLIC:
    ...
    /**
     *object to store msgid.
     */
//    class MsgIdObject: public cNamedObject{
//      public:
//      uint64_t msgDd;
//    }
    ...
    class SendController
    {
      PUBLIC:
        ...
        void processReceivedMsgACK(HomaPkt* rxPkt);
        void processMsgTimeoutSenderTimers(cMessage* msgTimeoutSenderTimer);
      ...
      PROTECTED:
        ...
        //timeout value for a sent message at sender side
        simtime_t msgTimeoutValSender;

        // The hash map from the msgId to outstanding messages.
        OutboundMsgMap outboundMsgMap;

        //The map from message id to message timeout timers
        std::unordered_map<uint64_t, cMessage *> msgTimeoutSenderTimerMap;

        //The map from message timeout timers to message id
        std::unordered_map<cMessage *, uint64_t> msgTimeoutSenderTimerToMsgIdMap;
    ...
    class InboundMessage
    {
      ...
      PROTECTED:
        ...
        uint32_t bytesToGrantLastRound;

        // Tracks the total number of grant bytes that the rxScheduler should
        // send for retransmitting the lost data of ehis message.
        uint32_t bytesToRetrans;
        ...
        // Tracks the next sequence number for this messaage that is to be sched        //uled for retransmission by next grant packet.
        uint32_t retrans_offset;
        ...
        //flag to track whether probe is received or not
        bool probeReceived;
        ...
        GrantList inflightRetransGrants;
      ...
      PROTECTED:
        ...
        HomaPkt* prepareRetransGrant(uint16_t grantSize, uint16_t schedPrio);
        HomaPkt* prepareMsgAck();
    ...
    class ReceiveScheduler
    {
      PUBLIC:
        ...
        class SenderState {
          PUBLIC:
            ...
            std::pair<bool, int> handleInboundProbe(HomaPkt* rxPkt);
          ...
          PROTECTED:
            ...
            //timeout value for a sent message at receiver side
            simtime_t msgTimeoutValReceiver;
        ...
        class SchedSenders {
          PUBLIC:
            ...
            void handleMsgTimeoutReceiverTimerEvent(SenderState* s, cMessage* msgTimeoutReceiverTimer); 
      ...
      PROTECTED:
        ...
        // mapping from msg timeout receiver timer to senderstate
        std::unordered_map<cMessage*, SenderState*> msgTimeoutReceiverTimerMap;
        // mapping from msg timeout receiver timer to msg ide
        std::unordered_map<cMessage*, uint64_t> msgTimeoutReceiverToMsgIdMap;
        // mapping from msg id to msg timeout receiver timer
        std::unordered_map<uint64_t, cMessage*> msgidToMsgTimeoutReceiverMap;
      ...
      PROTECTED:
        ...
        void processReceivedProbe(HomaPkt* rxPkt);
        void processGrantTimers(cMessage* grantTimer);
        void processMsgTimeoutReceiverTimers(cMessage* msgTimeoutReceiverTimer);
  ...
  PUBLIC:
    ...
    enum SelfMsgKind
    {
        ...
        MSGTIMEOUTSENDER = 7, //timeout for notifying packet retransmission of a msg at sender
        MSGTIMEOUTRECEIVER = 8  //timeout for notifying packet retransmission of a msg at receiver
    };
  ...
  PROTECTED:
    ...
    //Timeout timer for triggering message retransmission at the sender side
   // cMessage* msgTimeoutSenderTimer;

    //Timeout timer for triggering message retransmission at the receiver side
