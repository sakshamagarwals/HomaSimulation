...
std::ofstream outLoadFile;

std::string loadFileName = "susload/tmp.txt" ;



std::ofstream outFctFile;
std::string fctFileName = "fct_result/fct_size.txt" ; 
...
HomaTransport::initialize()
{
    ...
    nextEmitSignalTime = SIMTIME_ZERO;
   // msgTimeoutSenderTimer = new cMessage("msgTimeoutSenderTimer");
   // msgTimeoutReceiverTimer = new cMessage("msgTimeoutReceiverTimer");
    ...
    if (msg->isSelfMessage()) {
        switch (msg->getKind()) {
            ...
            case SelfMsgKind::BW_CHECK:
                ...
                break;
            case SelfMsgKind::MSGTIMEOUTSENDER:
                sxController.processMsgTimeoutSenderTimers(msg);
                break;
            case SelfMsgKind::MSGTIMEOUTRECEIVER:
                rxScheduler.processMsgTimeoutReceiverTimers(msg);
                break;
        }
    } else {
...
HomaTransport::handleRecvdPkt(cPacket* pkt)
{
    ...
    switch (rxPkt->getPktType()) {
        ...
        case PktType::UNSCHED_FIN_PROBE:
            rxScheduler.processReceivedProbe(rxPkt);
            break;
        ...
        case PktType::MSG_ACK:
            sxController.processReceivedMsgACK(rxPkt);
            break;
...
HomaTransport::SendController::SendController(HomaTransport* transport)
    ...
    , outboundMsgMap()
    , msgTimeoutSenderTimerMap()  
    , msgTimeoutSenderTimerToMsgIdMap()
    , rxAddrMsgMap()
    ...
    PriorityResolver* prioResolver)
{
    ...
    bytesLeftToSend = 0;
    bytesNeedGrant = 0;
    msgTimeoutValSender = SimTime(0.050);
...
HomaTransport::SendController::~SendController()
{
   ...
   //std::cout << "reach ~SendController point 1"<<endl;
    if (msgTimeoutSenderTimerMap.size()){
      for (auto it = msgTimeoutSenderTimerMap.begin(); it != msgTimeoutSenderTimerMap.end(); it++) {
        cMessage* msgTimeoutTimer = it->second;
        transport->cancelAndDelete(msgTimeoutTimer);
      }
    }
    //std::cout << "reach ~SendController point 2"<<endl;
...
/**
 * For every new ack of message completion that arrives from the receiver, 
 * this method is called to handle that ack.
 *
 * \param rxPkt
 *      receiverd message completion ack packet from the network.
 */

void
HomaTransport::SendController::processReceivedMsgACK(HomaPkt* rxPkt)
{
  EV << "processReceivedMsgACK point 1"<<endl;
  //std::cout << "processReceivedMsgACK start with msgId = "<< rxPkt->getMsgId()<<endl;

  OutboundMessage* outboundMsg = &(outboundMsgMap.at(rxPkt->getMsgId()));

  auto iter = msgTimeoutSenderTimerMap.find(outboundMsg->msgId);
  if (iter != msgTimeoutSenderTimerMap.end()) {
    cMessage* msgTimeoutTimer = msgTimeoutSenderTimerMap[outboundMsg->msgId];
    msgTimeoutSenderTimerToMsgIdMap.erase(msgTimeoutTimer);
    transport->cancelAndDelete(msgTimeoutTimer);
    msgTimeoutSenderTimerMap.erase(outboundMsg->msgId);

  }

  msgTransmitComplete(outboundMsg);
    
  EV << "processReceivedMsgACK point 2"<<endl;
  //std::cout << "processReceivedMsgACK end with msgId = "<< rxPkt->getMsgId()<<endl;
}
...
HomaTransport::SendController::processSendMsgFromApp(AppMessage* sendMsg)
{
    ...
    //std::cout << "SendController::processSendMsgFromApp start"<<endl;
    ...
    uint64_t appMsgLen = outboundMsg->getBytesLeft();
    ...
    //add timeout for msg retransmission at sender
    if (!this->transport->enableAeolus){
      cMessage* msgTimeoutSenderTimer = new cMessage("msgTimeoutSenderTimer");
      msgTimeoutSenderTimer->setKind(SelfMsgKind::MSGTIMEOUTSENDER);
      msgTimeoutSenderTimerMap[msgId] = msgTimeoutSenderTimer;
      msgTimeoutSenderTimerToMsgIdMap[msgTimeoutSenderTimer] = msgId;
      transport->scheduleAt(simTime() + msgTimeoutValSender, msgTimeoutSenderTimer);
    }
    ...
    delete sendMsg;
        ...
        sendOrQueue();
//std::cout << "SendController::processSendMsgFromApp end"<<endl;
}

/**
 * This method handles timeout of a message whose unscheduled packets are all lost.
 *
 * \param sendMsg
 *      Messsage that needs to be retransmitted by the transport.
 */
void
HomaTransport::SendController::processMsgTimeoutSenderTimers(cMessage* msgTimeoutSenderTimer)
{
    EV << "processMsgTimeoutSenderTimers point 1 " <<endl;
    // MsgIdObject* msgIdOb = msgTimeoutSenderTimer->getObject("msgIdObject");
    uint64_t msgId = msgTimeoutSenderTimerToMsgIdMap[msgTimeoutSenderTimer];
    EV << "processMsgTimeoutSenderTimers point 1 " <<endl;
    OutboundMessage* outboundMsg = &(outboundMsgMap.at(msgId));
    outboundMsg->bytesToSched = outboundMsg->msgSize;
    outboundMsg->bytesLeft = outboundMsg->msgSize;
    outboundMsg->unschedBytesLeft = 0;
    // std::cout << "sender timer: " << msgId << " msg size: " <<outboundMsg->msgSize << std::endl;
...
HomaTransport::SendController::processReceivedGrant(HomaPkt* rxPkt)
{
    ...
    auto miter = outboundMsgMap.find (rxPkt->getMsgId());
    if (miter == outboundMsgMap.end()) {
        return;
    }
    outboundMsg = &(miter->second);
    // if(outboundMsg->srcAddr.str() == "10.0.7.17" && outboundMsg->msgSize == 118260) {
    //     std::cout << simTime() << " " << "rxPkt->getGrantFields().bytesToGrant: " << rxPkt->getGrantFields().bytesToGrant << std::endl;
    //     std::cout << simTime() << " " << "outboundMsg->bytesToSched: " << outboundMsg->bytesToSched << std::endl;
    //     std::cout << simTime() << " " << "outboundMsg->bytesLeft " << outboundMsg->bytesLeft << std::endl;
    ...
    if (rxPkt->getGrantFields().bytesToGrant > outboundMsg->bytesToSched){
      this->bytesLeftToSend += rxPkt->getGrantFields().bytesToGrant - outboundMsg->bytesToSched;
      outboundMsg->bytesToSched = rxPkt->getGrantFields().bytesToGrant;
      outboundMsg->bytesLeft = outboundMsg->bytesToSched;
    }
    ...
    // ASSERT(insResult.second); // check insertion took place
    // if(outboundMsg->srcAddr.str() == "10.0.7.17" && outboundMsg->msgSize == 118260) {
    //     std::cout << simTime() << " " << "receive grant " << rxPkt->getGrantFields().offset << std::endl;
    //         std::cout << simTime() << " " << "outboundMsg->bytesLeft " << outboundMsg->bytesLeft << std::endl;
    // }
    delete rxPkt;
    sendOrQueue();
    //std::cout << "processReceivedGrant end "<<endl;
}
...
{
    HomaPkt* sxPkt = NULL;
    // if(srcAddr.str() == "10.0.7.17") {
    //     std::cout << simTime() << "call sendorqueue" << std::endl;
    //     std::cout << "outbndMsgSet.size: " << outbndMsgSet.size() << std::endl;
    // }
    if (msg == transport->sendTimer) {
        ...
        if (!outGrantQueue.empty()) {
            ...
            // if(srcAddr.str() == "10.0.7.17") {
            //     std::cout << simTime() << "send a grant \n" << std::endl;
            // }
            return;
        }
        // if(srcAddr.str() == "10.0.7.17") {
        //     std::cout << simTime() << "reach here 1\n" << std::endl;
        // }
        ...
        if (!outbndMsgSet.empty()) {
           ...
           // if(highPrioMsg->srcAddr.str() == "10.0.7.17") {
           //      srcAddr = highPrioMsg->srcAddr;
           //      std::cout << simTime() << "reach here 2\n" << std::endl;
           //      std::cout << simTime() << " " << "high priority message size: " << highPrioMsg->msgSize << std::endl;
           //      std::cout << simTime() << " " << "byte left " << highPrioMsg->bytesLeft << std::endl;
           //  } 

            // if(highPrioMsg->srcAddr.str() == "10.0.7.17" && highPrioMsg->msgSize == 118260) {
            //     std::cout << simTime() << " " << "select: " << highPrioMsg->msgSize << std::endl;
            // }
            if (!outboundMsgMap.count(highPrioMsg->msgId)) {
                numRemoved = 0;
            }

            bool hasMoreReadyPkt;
            if (numRemoved > 1) {
                if(highPrioMsg->txPkts.empty())
                    hasMoreReadyPkt = false;
                else{
                    hasMoreReadyPkt = highPrioMsg->getTransmitReadyPkt(&sxPkt);
                    sendPktAndScheduleNext(sxPkt);
                    // if(highPrioMsg->srcAddr.str() == "10.0.7.17" && highPrioMsg->msgSize == 118260) {
                    //     std::cout << simTime() << " " << "send data pkt offset:" << sxPkt->getSchedDataFields().firstByte << std::endl;
                    // }
                }
            }
    ...
    if (sxPkt && sxPkt->getPktType() == PktType::GRANT) {
    ...
    // When this function is called to send a message ack packet.
    if (sxPkt && sxPkt->getPktType() == PktType::MSG_ACK) {
      ASSERT(sxPkt->getPktType() == PktType::MSG_ACK);
      if (transport->sendTimer->isScheduled()) {
        // NIC tx link is busy sending another packet
        EV << "Message ack is scheduled! Message ack to " <<
          sxPkt->getDestAddr().toIPv4().str() << ", mesgId: "  <<
          sxPkt->getMsgId() << " is queued!"<< endl;
        ...
        ASSERT(outGrantQueue.empty());
        EV << "Send message ack to: " << sxPkt->getDestAddr().toIPv4().str()
          << ", mesgId: "  << sxPkt->getMsgId() << ", prio: " <<
          sxPkt->getGrantFields().schedPrio<< endl;
         EV << "test point 1: PktType = " << sxPkt->getPktType() << endl;
        sendPktAndScheduleNext(sxPkt);
        EV << "test point 2 "<< endl; 
        return;
      }
    }
    ...
    size_t numRemoved = 0;
    if (eraseType == 0) {
        numRemoved =  outbndMsgSet.erase(highPrioMsg);
    }
    else if (eraseType == 1) {
        outbndMsgSet.erase(outbndMsgSet.begin());
        numRemoved = 1;
    }
...
{
    //EV << "sendPktAndScheduleNext: part 0"<< endl;
    PktType pktType = (PktType)sxPkt->getPktType();
    //EV << "sendPktAndScheduleNext: part 0.5"<< endl;
...
// EV << "sendPktAndScheduleNext: part 0.5"<< endl;
    ...
    simtime_t timeNow = simTime();
   // EV << "sendPktAndScheduleNext: part 1"<< endl;
    ...
    switch(pktType)
    {
        ...
        case PktType::MSG_ACK:
            ...
            //EV << "sendPktAndScheduleNext: part 2"<< endl;
            break;
        case PktType::UNSCHED_FIN_PROBE:
            break;
    ...
    //EV << "sendPktAndScheduleNext: part 3"<< endl;
...
{
    //std::cout << "SendController::msgTransmitComplete start "<<endl;
  ...
  //  std::cout << "SendController::msgTransmitComplete end "<<endl;
    ...
    switch (msg1->homaConfig->getSenderScheme()) {
        ...
        case HomaConfigDepot::SenderScheme::SRBF:
            ...
            return std::make_tuple(
                msg1->getBytesLeft(), msg1->getMsgCreationTime(), msg1->getMsgId()
                ) < 
                std::make_tuple (
                msg2->getBytesLeft(), msg2->getMsgCreationTime(), msg2->getMsgId()   
                );
...
HomaTransport::OutboundMessage::prepareRequestAndUnsched()
{
    ...
    //std::cout << "prepareRequestAndUnsched start "<<endl;
    ...
    EV << "prepareRequestAndUnsched: totalUnschedBytes = " << totalUnschedBytes <<endl;
    ...
    if (this->sxController->transport->enableAeolus){
      PktType pktType = PktType::UNSCHED_FIN_PROBE;
      HomaPkt* probePkt = new HomaPkt(sxController->transport);
      probePkt->setPktType(pktType);
      probePkt->setDestAddr(this->destAddr);
      probePkt->setSrcAddr(this->srcAddr);
      probePkt->setMsgId(this->msgId);
      probePkt->setPriority(unschedPrioVec[0]);
      ...
      unschedFields.totalUnschedBytes = totalUnschedBytes;
      unschedFields.firstByte = nextByteToSched;
      unschedFields.lastByte = -1;
    //  unschedFields.prioUnschedBytes = NULL;
      probePkt->setUnschedFields(unschedFields);
      probePkt->setByteLength(probePkt->headerSize());

      txPkts.push(probePkt);
      EV <<"probe pkt prpared, msgid = "<< probePkt->getMsgId()<<"  total unscheduledbytes = " << totalUnschedBytes <<endl;
    }
  //  std::cout << "prepareRequestAndUnsched end "<<endl;
}
...
{
    //std::cout << "OutboundMessage::prepareSchedPkt start "<<endl;
 ...
 //   std::cout << "OutboundMessage::prepareSchedPkt end "<<endl;
...
HomaTransport::OutboundMessage::getTransmitReadyPkt(HomaPkt** outPkt)
{
    ...
    *outPkt = head;
    ...
    bytesLeft = bytesLeft > numDataBytes ? bytesLeft - numDataBytes : 0;

    if (outPktType == PktType::REQUEST || outPktType == PktType::UNSCHED_DATA) {
        unschedBytesLeft = unschedBytesLeft > numDataBytes ? unschedBytesLeft - numDataBytes : 0;
    }
    ...
    const HomaPkt* pkt2)
{
    ...
    switch (txScheme) {
        ...
        case HomaConfigDepot::SenderScheme::SRBF:
            ...
            //     pkt1->getPriority() > pkt2->getPriority()));
            return std::make_tuple(
                pkt1->getFirstByte(), pkt1->getCreationTime(), pkt1->getPriority()
                ) > 
                std::make_tuple (
                pkt2->getFirstByte(), pkt2->getCreationTime(), pkt2->getPriority()   
                );
        default:
                throw cRuntimeError("Undefined SenderScheme parameter");
    }
...
HomaTransport::ReceiveScheduler::ReceiveScheduler(HomaTransport* transport)
    ...
    , ipSendersMap()
    , grantTimersMap()
    , msgTimeoutReceiverTimerMap()
    , msgTimeoutReceiverToMsgIdMap()
    , msgidToMsgTimeoutReceiverMap()
    , schedSenders(NULL)
...
{
  //std::cout<<"~ReceiveScheduler() start"<<endl;
    ...
    if ( msgTimeoutReceiverTimerMap.size() ){
      for (auto it = msgTimeoutReceiverTimerMap.begin(); it != msgTimeoutReceiverTimerMap.end(); it++) {
        cMessage* msgTimeoutTimer = it->first;
        transport->cancelAndDelete(msgTimeoutTimer);
      }
    }
...
for (auto it = ipSendersMap.begin(); it != ipSendersMap.end(); it++) {
    ...
    //std::cout << "reach ~ReceiveScheduler end"<<endl;
 ...
 */
void
HomaTransport::ReceiveScheduler::processReceivedProbe(HomaPkt* rxPkt)
{
  //std::cout << "ReceiveScheduler::processReceivedProbe start with msgId = " <<  rxPkt->getMsgId()<<endl;
  uint32_t pktLenOnWire = HomaPkt::getBytesOnWire(rxPkt->getDataBytes(),
      (PktType)rxPkt->getPktType());
  simtime_t pktDuration =
    SimTime(1e-9 * (pktLenOnWire * 8.0 / homaConfig->nicLinkSpeed));
  ...
  auto msgCompHandle = s->handleInboundProbe(rxPkt);
//  std::cout << "finish handle inbound probe" << endl;
  // Handling a received probe can change the state of the scheduled senders.
  ...
  //std::cout << "ReceiveScheduler::processReceivedProbe end with msgId = " <<  rxPkt->getMsgId()<<endl;
  delete rxPkt;
    return;
}
...
{ 
  //std::cout << "ReceiveScheduler::processReceivedPkt start with msgId= " << rxPkt->getMsgId() << endl;
    uint32_t pktLenOnWire = HomaPkt::getBytesOnWire(rxPkt->getDataBytes(),
        (PktType)rxPkt->getPktType());
    simtime_t pktDuration =
        SimTime(1e-9 * (pktLenOnWire * 8.0 / homaConfig->nicLinkSpeed));
    ...
    /**
     * statistics: goodput, sustainable load
     */
    if (enableSustainable) {

        // statistic version 2, period, record from the simulation start
        recStart = std::min (recStart, timeNow.dbl());
        recEnd = std::max (recEnd, timeNow.dbl());
        totalByteRecv += rxPkt->getDataBytes();
        totalGoodput = (double) totalByteRecv / (recEnd - recStart);
        totalGoodput = totalGoodput * 8.0 / 1000000000.0; // Gbps
        totalGoodput = totalGoodput / 144; // 144 host
        outputCount += 1;
        outputCount %= outputInterval;
        if (outputCount == 0) {
            maxGoodput = std::max (maxGoodput, totalGoodput);
            if (enableWriteLoad) {
                outLoadFile << totalGoodput 
                    << "\t " << (maxGoodput)
                    << "\t " << (recStart)
                    << "\t " << (recEnd - recStart) 
                    << endl;
    ...
    if (sinceLastPkt > (pktDuration + rxGrantsTime) &&
        ...
        simtime_t maxEarliestGrant = SIMTIME_ZERO;
    ...
    *sumGrantsInGap = 0;
    ...
    auto mesgIt = s->incompleteMesgs.find(rxPkt->getMsgId());
    if ( (mesgIt == s->incompleteMesgs.end()) && (rxPkt->getPktType() == PktType::SCHED_DATA) ){
      delete rxPkt;
      return;
    }
  ...
  //std::cout << "ReceiveScheduler::processReceivedPkt end" << endl;
}

/**
 * This method handles timeout of a message at receiver side.
 *
 * \param msgTimeoutReceiverTimer
 *      Timer of message that needs packet retransmission.
 */
void
HomaTransport::ReceiveScheduler::processMsgTimeoutReceiverTimers(cMessage* msgTimeoutReceiverTimer)
{   
  //std::cout << "ReceiveScheduler::processMsgTimeoutReceiverTimers start" << endl;

    EV << "\n\n################Process Msg Timeout Timer at Receiver Side################\n\n" << endl;
    SenderState* s = msgTimeoutReceiverTimerMap[msgTimeoutReceiverTimer];
    schedSenders->handleMsgTimeoutReceiverTimerEvent(s, msgTimeoutReceiverTimer);
  //std::cout << "ReceiveScheduler::processMsgTimeoutReceiverTimers end" << endl;
...
{
  //std::cout << "ReceiveScheduler::processGrantTimers start" << endl;
    ...
    // signal for activeSenders.
    tryRecordActiveMesgStats(timeNow);
  //std::cout << "ReceiveScheduler::processGrantTimers end" << endl;
}
...
{
  msgTimeoutValReceiver = SimTime(0.0003); // 300us
}
...
/**
 * This method handles probe packets received from the sender corresponding to
 * SenderState. It finds the sender's InboundMessage that this belongs to and
 * detect the lost unscheduled packets. 
 *
 * \param rxPkt
 *      The probe packet received from the sender.
 * \return
 ...
 */
std::pair<bool, int>
HomaTransport::ReceiveScheduler::SenderState::handleInboundProbe(HomaPkt* rxPkt)
{
//   std::cout << "SenderState::handleInboundProbe start with msgId = " << rxPkt->getMsgId() <<endl;
  ...
  if (mesgIt == incompleteMesgs.end()) {
    ASSERT(rxPkt->getPktType() == PktType::UNSCHED_FIN_PROBE);
...
// EV << "handleInboundProbe: reach point 2." << endl;

  inboundMesg->probeReceived = true;
  //update data status of this message caused by data loss
  uint32_t bytesReceived = inboundMesg->msgSize - inboundMesg->bytesToReceive;
  uint32_t lostUnschedBytes = inboundMesg->totalUnschedBytes - bytesReceived;
  inboundMesg->bytesToGrant += lostUnschedBytes;
  inboundMesg->totalBytesInFlight -= HomaPkt::getBytesOnWire(lostUnschedBytes, PktType::UNSCHED_FIN_PROBE); 
  inboundMesg->retrans_offset = 0;
  inboundMesg->bytesToRetrans = lostUnschedBytes;
  InboundMessage::GrantList::iterator grantZero = inboundMesg->inflightGrants.begin();
  if (grantZero != inboundMesg->inflightGrants.end()){
    inboundMesg->inflightGrants.erase(grantZero);
  }
  inboundMesg->totalUnschedBytes -= lostUnschedBytes;

//EV << "handleInboundProbe: reach point 4." << endl;
...
{
 // std::cout << "SenderState::handleInboundPkt start with msgId = " << rxPkt->getMsgId() <<endl;
    ...
    if (mesgIt == incompleteMesgs.end()) {
        EV << "rxPkt->getPktType() = "<<rxPkt->getPktType()<<endl;
        EV << "rxPkt->getMsgId() = "<<rxPkt->getMsgId()<<endl;
        ASSERT(rxPkt->getPktType() == PktType::REQUEST ||
            rxPkt->getPktType() == PktType::UNSCHED_DATA);
        inboundMesg = new InboundMessage(rxPkt, this->rxScheduler, homaConfig);
        incompleteMesgs[rxPkt->getMsgId()] = inboundMesg;

        cMessage* msgTimeoutReceiverTimer = new cMessage("msgTimeoutReceiverTimer");
        msgTimeoutReceiverTimer->setKind(SelfMsgKind::MSGTIMEOUTRECEIVER); 
        this->rxScheduler->msgTimeoutReceiverTimerMap[msgTimeoutReceiverTimer] = this; 
        this->rxScheduler->msgTimeoutReceiverToMsgIdMap[msgTimeoutReceiverTimer] = rxPkt->getMsgId();
        this->rxScheduler->msgidToMsgTimeoutReceiverMap[rxPkt->getMsgId()] = msgTimeoutReceiverTimer;

        //MsgIdObject* msgIdOb = new MsgIdObject("msgIdObject");
        //msgIdOb->msgId = msgId;
       // msgTimeoutReceiverTimer->addObject(msgIdOb);
        this->rxScheduler->transport->scheduleAt(simTime() + msgTimeoutValReceiver, msgTimeoutReceiverTimer);
    ...
    if (pktType == PktType::REQUEST || pktType == PktType::UNSCHED_DATA) {
        //std::cout<<"receive REQUEST or UNSCHED pkt"<<endl;
     ...
     // std::cout<<"receive SCHED pkt"<<endl;
    ...
    if (inboundMesg->bytesToReceive == 0) {
       ...
       // ASSERT(inboundMesg->inflightGrants.empty());
        ...
        // this message is complete, so send it to the application
        cMessage * msgTimeoutReceiverTimer = this->rxScheduler->msgidToMsgTimeoutReceiverMap[inboundMesg->msgIdAtSender];
        this->rxScheduler->transport->cancelAndDelete(msgTimeoutReceiverTimer);
        this->rxScheduler->msgidToMsgTimeoutReceiverMap.erase(inboundMesg->msgIdAtSender);
        this->rxScheduler->msgTimeoutReceiverToMsgIdMap.erase(msgTimeoutReceiverTimer);
        this->rxScheduler->msgTimeoutReceiverTimerMap.erase(msgTimeoutReceiverTimer);

        if ( (this->rxScheduler->transport->enableAeolus && inboundMesg->probeReceived) || (!this->rxScheduler->transport->enableAeolus) ){
        AppMessage* rxMsg = inboundMesg->prepareRxMsgForApp();
#if TESTING
          delete rxMsg;
#else
  ...
  //std::cout << "SenderState::handleInboundPkt end with msgId = " << rxPkt->getMsgId() <<endl;
    return ret;
}
        ...
        uint32_t grantPrio)
{
    ...
    uint16_t grantSize;
    HomaPkt* grantPkt;
    if ( rxScheduler->transport->enableAeolus && (topMesg->bytesToRetrans > 0) ){
      grantSize = std::min(topMesg->bytesToRetrans, homaConfig->grantMaxBytes);
      grantPkt = topMesg->prepareRetransGrant(grantSize, grantPrio);
    }else{
      grantSize = std::min(topMesg->bytesToGrant, homaConfig->grantMaxBytes);
      grantPkt = topMesg->prepareGrant(grantSize, grantPrio);
    }
    ...
    // if(topMesg->destAddr.str() == "10.0.7.41") {
    //     std::cout << currentTime << " " << "send grant for flow size " << topMesg->msgSize <<" " << grantPkt->getGrantFields().offset << std::endl;
    // }
...
/**
 * This method process per message timeout timers at receiver side.
 */
void
HomaTransport::ReceiveScheduler::SchedSenders::handleMsgTimeoutReceiverTimerEvent(
    SenderState* s, cMessage* msgTimeoutReceiverTimer)
{
    // scheduled packetd will not be dropped due to the property of receiver-driven protocol.
    // this rarelly happen with homa's credit allocation method.
     return;

    uint64_t msgId = this->rxScheduler->msgTimeoutReceiverToMsgIdMap[msgTimeoutReceiverTimer];
    //MsgIdObject* msgIdOb = msgTimeoutReceiverTimer->getObject("msgIdObject");
    InboundMessage* timeoutMsg = s->incompleteMesgs.find(msgId)->second;

    if ( (timeoutMsg->bytesToGrant > 0) ){
      this->rxScheduler->transport->scheduleAt(simTime() + s->msgTimeoutValReceiver, msgTimeoutReceiverTimer);
    //  timeoutMsg->bytesToGrantLastRound = timeoutMsg->bytesToGrant;
      return;
    }

    timeoutMsg->bytesToGrant = 0;
    timeoutMsg->bytesToReceive = 0;
    timeoutMsg->bytesGrantedInFlight = 0;
    timeoutMsg->totalBytesInFlight = 0;

    AppMessage* rxMsg = timeoutMsg->prepareRxMsgForApp();
    ...
    if ( (timeoutMsg->msgSize > timeoutMsg->totalUnschedBytes) && (numToGrant > homaConfig->numSendersToKeepGranted) ){
      numToGrant--;
    }

    s->incompleteMesgs.erase(timeoutMsg->msgIdAtSender);
    auto searchMsg = s->mesgsToGrant.find(timeoutMsg);
    if (searchMsg != s->mesgsToGrant.end()){
      s->mesgsToGrant.erase(timeoutMsg);
    }
    ...
    , unschedToRecvPerPrio()
{
    switch (rxPkt->getPktType()) {
        ...
        case PktType::UNSCHED_DATA:
            ...
            bytesToGrantLastRound = bytesToGrant;
        ...
        uint32_t byteEnd, PktType pktType)
{
    ...
    if (pktType == PktType::UNSCHED_DATA || pktType == PktType::REQUEST) {
     ...
     //   ASSERT(std::get<1>(*grantZero) >= bytesReceived);
        if (std::get<1>(*grantZero) < bytesReceived){
          std::get<1>(*grantZero) = 0;
        }else{
          std::get<1>(*grantZero) -= bytesReceived;
        }
        if ( (std::get<1>(*grantZero) == 0) && (!this->rxScheduler->transport->enableAeolus) ) {
    ...
    if (pktType == PktType::SCHED_DATA) {
        ...
        bool foundGrantInRegrant = false;
        if (this->rxScheduler->transport->enableAeolus){
          for (grant = inflightRetransGrants.begin(); grant != inflightRetransGrants.end(); grant++) {
            if (std::get<0>(*grant) == byteStart) {
              foundGrantInRegrant = true;
              break;
            }
          }
          if (foundGrantInRegrant) {
          ASSERT(grant != inflightRetransGrants.end());
          inflightRetransGrants.erase(grant);
          }
          //bytesToRetrans -= bytesReceived;
        }
        if (!foundGrantInRegrant){
          for (grant = inflightGrants.begin(); grant != inflightGrants.end(); grant++) {
            if (std::get<0>(*grant) == byteStart) {
              break;
            }
          }
   ...
   // std::cout << "Received " << bytesReceived << " bytes from " << srcAddr.str()
     //       << " for msgId " << msgIdAtSender << " (" << bytesToReceive <<
       //     " bytes left to receive)" << endl;
...
/**
 * Creates a grant packet for this message to retransmit lost packet and update the internal structure of
 * the message.
 *
 * \param grantSize
 ...
 */
HomaPkt*
HomaTransport::InboundMessage::prepareRetransGrant(uint16_t grantSize,
    uint16_t schedPrio)
{
    ...
    grantFields.offset = retrans_offset;
    grantFields.grantBytes = grantSize;
    grantFields.schedPrio = schedPrio;
    grantFields.bytesToGrant = this->bytesToGrant;
    ...
    this->inflightRetransGrants.push_back(
        std::make_tuple(retrans_offset, grantSize, simTime()));
    this->bytesToGrant = this->bytesToGrant > grantSize? this->bytesToGrant - grantSize: 0;
    this->bytesToRetrans = this->bytesToRetrans > grantSize? this->bytesToRetrans - grantSize : 0;
    this->retrans_offset += grantSize;
   ...
   // ASSERT(this->offset == this->msgSize - this->bytesToGrant);
    ...
    ASSERT(this->offset - this->bytesToRetrans == this->msgSize - this->bytesToGrant);
 ...
 */
HomaPkt*
HomaTransport::InboundMessage::prepareGrant(uint16_t grantSize,
    uint16_t schedPrio)
{
    // prepare a grant
    HomaPkt* grantPkt = new HomaPkt(rxScheduler->transport);
...
/**
 * Creates a completion ack packet for this message and update the internal structure of
 * the message.
 *
 * \return
 *      Ack packet that is to be sent on the wire to the sender of this
 *      message.
 */
HomaPkt*
HomaTransport::InboundMessage::prepareMsgAck()
{

    // prepare a grant
    HomaPkt* msgAckPkt = new HomaPkt(rxScheduler->transport);
    msgAckPkt->setPktType(PktType::MSG_ACK);
    msgAckPkt->setDestAddr(this->srcAddr);
    msgAckPkt->setSrcAddr(this->destAddr);
    msgAckPkt->setMsgId(this->msgIdAtSender);
    msgAckPkt->setByteLength(msgAckPkt->headerSize());
    msgAckPkt->setPriority(0);
...
HomaTransport::InboundMessage::prepareRxMsgForApp()
{
    ...
    // std::cout<<this->msgSize<<" "<<SIMTIME_DBL(simTime() - this->msgCreationTime)<<std::endl;
    ...
    HomaPkt* msgAckPkt = this->prepareMsgAck();
    this->rxScheduler->transport->sxController.sendOrQueue(msgAckPkt);
    return rxMsg;
}
...
HomaTransport::InboundMessage::copy(const InboundMessage& other)
{
    ...
    this->bytesToGrantLastRound = other.bytesToGrantLastRound;
